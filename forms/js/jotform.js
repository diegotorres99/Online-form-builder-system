    ...
    checkCalculation: function (calc) {
        ...
        switch (resultFieldType) {
                ...
                if (combinedObject && "year" in combinedObject || resultFieldType === 'time') {
                    if (output.length === 13) {
                        ...
                        if (!isNaN(hour) && !isNaN(minute)) {
                            ...
                            $("input_" + result+ "_hourSelect").value = hour;
                            $("input_" + result+ "_minuteSelect").value = minute;
                        }
                    } else {
                        for (var inputId in combinedObject) {
                    ...
                    try {
                        if ((typeof output == "number" && output > 0) || (typeof output == "string" && output.replace(/\s/g, "").length > 0 && output !== "0")) {
                            ...
                            if ($('input_'+result)) {
                              ...
                              if (!isNaN(hour) && !isNaN(minute)) {
                                var ampmField = $('ampm_'+result);

                                if (ampmField) {
                                    $('ampm_'+result).value = hour > 12 ? 'PM' : 'AM';
                                    hour = hour > 12 ? hour - 12 : hour;
                                }

                                $("hour_" + result).value = hour;
                                $("min_" + result).value = minute;
                              }
                            }
                        }
                    } catch (e) {
            ...
            case "email":
                ...
                break;
            case "appointment":
                const parsedOutput = calc.conditionTrue ? parseInt(output) : false;
   ...
   */
    ...
    setFieldValidation: function (input) {
        ...
        if (input.type == 'checkbox' || input.type == 'radio') {
            ...
            const vals = JotForm.getInputValidations(input);
